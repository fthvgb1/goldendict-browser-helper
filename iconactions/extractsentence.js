;const {ankiFetchClickFn, getAnkiFetchParams, arrayDiff, superFetchHook, setEleDrag} = (() => {
    ...
    const mapTitle = {
        ...
        'from-field': 'anki字段名，将提取该字段的内容或用于后续用于query的元素,$doc表示document,*表示任意字段',
        'to-field': 'anki字段名，将提取的内容赋值给该字段,$doc表示document,*表示任意字段,*|scope, scope值为html和text,表示只针对富文本或者纯文本字段',
        'handle': '处理',
        'concatenation': '拼接',
        ...
        'parseTemplate': '对某个值或变量进行替换字符串',
        'templateVar': '要替换的值或变量',
        'toUpperCase': '转成大写',
        'toLowerCase': '转成小写',
        'separator': '分隔符',
        'cached': '缓存该值(只查询一次)',
    ...
    return {
        ...
        superFetchHook: {
            ...
            hookLang: langKV => Object.assign(mapTitle, langKV),
